...
export async function createOrder(sessionId) {
  return paypal('/v2/checkout/orders', {
    method: 'POST',
    idempotencyKey: requestId('ord', sessionId),
    body: {
      intent: 'CAPTURE',
      purchase_units: [{
        ...
        amount: {
          currency_code: PRODUCT.currency, value: PRODUCT.price,
          breakdown: { item_total: { currency_code: PRODUCT.currency, value: PRODUCT.price } },
        },
        items: [{ name: PRODUCT.name, sku: PRODUCT.id, quantity: '1', category: 'DIGITAL_GOODS', unit_amount: { currency_code: PRODUCT.currency, value: PRODUCT.price } }],
      }],
...
function correctAmount(amount) {
  return amount?.currency_code === PRODUCT.currency && typeof amount.value === 'string' && /^29(?:\.0{1,2})?$/.test(amount.value);
}
...
export function verifyOrder(order, session) {
  ...
  if (order.id !== session.orderId || order.intent !== 'CAPTURE' || order.purchase_units?.length !== 1 ||
    ...
    !correctAmount(unit.amount) || unit.items?.length !== 1 || item.sku !== PRODUCT.id ||
    item.quantity !== '1' || !correctAmount(item.unit_amount)) {
...
export function verifyCapture(capture, orderId, captureId) {
  const relatedOrder = capture.supplementary_data?.related_ids?.order_id;
  if (!ID_PATTERN.test(capture.id || '') || (captureId && capture.id !== captureId) || !correctAmount(capture.amount) ||
...
export async function captureOrder(session) {
  ...
  verifyCapture(summary, session.orderId);
  const capture = await currentCapture(summary.id, session.orderId);
  return { order, capture };
}

export async function currentCapture(captureId, orderId) {
  const capture = await paypal(`/v2/payments/captures/${validateId(captureId)}`);
  return verifyCapture(capture, orderId, captureId);
}

export function accessPayload(orderId, captureId) {
  return { orderId, captureId, notionUrl: requireConfiguration().notionUrl, eventId: `purchase_${captureId}`, price: PRODUCT.price, currency: PRODUCT.currency };
}

export function grantAccess(res, session, capture) {
  setToken(res, 'access', { orderId: session.orderId, captureId: capture.id, productId: PRODUCT.id }, ACCESS_AGE);
  return accessPayload(session.orderId, capture.id);
}
...
export async function sendPurchase(req, session, order, capture) {
  ...
  const event = {
    ...
    user_data: userData,
    custom_data: { currency: PRODUCT.currency, value: Number(PRODUCT.price), content_ids: [PRODUCT.id], content_type: 'product', num_items: 1, order_id: session.orderId },
  };
  try {
