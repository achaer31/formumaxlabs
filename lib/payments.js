...
export function boundPrice(session) {
  // Existing signed orders/access cookies from the original USD29 launch remain valid.
  const price = session?.price ?? PRODUCT.price;
...
function correctAmount(amount, price = PRODUCT.price) {
  return amount?.currency_code === PRODUCT.currency && typeof amount.value === 'string' && /^\d+(?:\.\d{1,2})?$/.test(amount.value) && Number(amount.value) === Number(boundPrice({ price }));
}
...
export function verifyCapture(capture, orderId, captureId, price = PRODUCT.price) {
...
export async function currentCapture(captureId, orderId, price = PRODUCT.price) {
  const capture = await paypal(`/v2/payments/captures/${validateId(captureId)}`);
  return verifyCapture(capture, orderId, captureId, price);
}

export function accessPayload(orderId, captureId, price = PRODUCT.price) {
