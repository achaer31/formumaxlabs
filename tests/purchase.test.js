...
async function setup({existing=false,pending=false,consent=false,badConfig=false,badUrl=false,failedSdk=false,price='19.00',acceptedPrice=null}={}){
  ...
  const fetchMock=async(url,options)=>{
    ...
    if(url==='/api/config')return Response.json({productId:'ultimate-video-ai-mastery',price:badConfig?'1.00':state.price,regularPrice:'29.00',offerPrice:'19.00',offerActive:state.price==='19.00',offerExpiresAt:state.offerExpiresAt||new Date(Date.now()+(state.price==='19.00'?18000000:-1000)).toISOString(),serverTime:new Date(Date.now()).toISOString(),acceptedOrder:state.acceptedPrice?{orderId:ORDER,price:state.acceptedPrice,currency:'USD'}:null,currency:'USD',checkoutAvailable:true,paypalClientId:'mock-client',pixelId:'1467482891263700'});
...
test('first advertising consent queues PageView and ViewContent; revoked consent blocks Purchase',async()=>{
  const{mod,state}=await setup({consent:true});await mod.initTracking();assert.ok(state.events.some(e=>e[0]==='track'&&e[1]==='PageView'));assert.ok(state.events.some(e=>e[0]==='track'&&e[1]==='ViewContent'));assert.equal(state.events.find(e=>e[1]==='ViewContent')[2].value,19);await mod.initCheckout();await state.paypalOptions.createOrder();mod.setConsent(false);await state.paypalOptions.onApprove({orderID:ORDER},{});assert.equal(state.calls.find(c=>c.url==='/api/capture').body.consent,false);assert.equal(state.events.some(e=>e[1]==='Purchase'),false);
});
...
test('countdown reaches zero, hides promotion, displays USD29 and rechecks the server',async()=>{
  ...
  const cleanup=await mod.initOffer();assert.equal(offerNodes.price.textContent,'$19');assert.equal(offerNodes.countdown.textContent,'00:00:01');
  Date.now=()=>now+1100;state.price='29.00';state.offerTick();await Promise.resolve();await Promise.resolve();
  assert.equal(offerNodes.price.textContent,'$29');assert.equal(offerNodes.countdown.textContent,'00:00:00');assert.equal(offerNodes.promo.hidden,true);
...
test('price expiry during checkout requires review before any order is created',async()=>{
  const{mod,state,map}=await setup();await mod.initCheckout();state.price='29.00';
  ...
  assert.equal(state.calls.find(call=>call.url==='/api/orders').body.expectedPrice,'29.00');
});
test('USD29 purchases report the captured value and display the actual receipt total',async()=>{
  const{mod,state,map}=await setup({consent:true,price:'29.00'});await mod.initCheckout();await state.paypalOptions.createOrder();await state.paypalOptions.onApprove({orderID:ORDER},{});
  assert.equal(state.events.find(event=>event[1]==='Purchase')[2].value,29);
  assert.ok(map.get('#checkout-content').children.some(node=>node.textContent?.includes('Paid US$29')));
});
test('an existing discounted order stays USD19 while public course labels show USD29',async()=>{
  const{mod,state,offerNodes}=await setup({price:'29.00',acceptedPrice:'19.00'});await mod.initCheckout();await state.paypalOptions.createOrder();
  assert.equal(offerNodes.price.textContent,'$29');assert.equal(state.calls.find(call=>call.url==='/api/orders').body.expectedPrice,'19.00');
});

test('initial price lookup failure clears stale promotional amounts and retries when the page regains focus',async()=>{
  const{mod,state,offerNodes}=await setup({price:'29.00'});offerNodes.price.textContent='$19';state.configFailure=true;
  ...
  assert.equal(offerNodes.price.textContent,'$29');assert.equal(offerNodes.promo.hidden,true);
...
test('a historical USD99 amount cannot be advertised as the current checkout price',async()=>{
  const{mod,state,map}=await setup({price:'99.00'});await mod.initCheckout();
...
test('a server-accepted historical USD99 order keeps its amount through capture and Meta Purchase',async()=>{
  const{mod,state,map,offerNodes}=await setup({consent:true,price:'19.00',acceptedPrice:'99.00'});
  await mod.initCheckout();await state.paypalOptions.createOrder();await state.paypalOptions.onApprove({orderID:ORDER},{});
  assert.equal(offerNodes.price.textContent,'$19');
...
test('verified historical USD99 access displays the original receipt without a new order or replayed Purchase',async()=>{
  const{mod,state,map}=await setup({existing:true,consent:true,price:'29.00',acceptedPrice:'99.00'});
