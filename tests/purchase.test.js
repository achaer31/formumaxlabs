...
async function setup({existing=false,pending=false,consent=false,badConfig=false,badUrl=false,failedSdk=false,price='9.99',acceptedPrice=null}={}){
  ...
  const fetchMock=async(url,options)=>{
    ...
    if(url==='/api/config')return Response.json({productId:'ultimate-video-ai-mastery',price:badConfig?'1.00':state.price,regularPrice:'19.99',offerPrice:'9.99',offerActive:state.price==='9.99',offerExpiresAt:state.offerExpiresAt||new Date(Date.now()+(state.price==='9.99'?18000000:-1000)).toISOString(),serverTime:new Date(Date.now()).toISOString(),acceptedOrder:state.acceptedPrice?{orderId:ORDER,price:state.acceptedPrice,currency:'USD'}:null,currency:'USD',checkoutAvailable:true,paypalClientId:'mock-client',pixelId:'1467482891263700'});
...
test('concurrent checkout opens and order callbacks render/create only once',async()=>{
  ...
  const ids=await Promise.all([state.paypalOptions.createOrder(),state.paypalOptions.createOrder()]);assert.deepEqual(ids,[ORDER,ORDER]);assert.equal(state.calls.filter(c=>c.url==='/api/orders').length,1);assert.equal(state.calls.find(c=>c.url==='/api/orders').body.expectedPrice,'9.99');
});
...
test('first advertising consent queues PageView and ViewContent; revoked consent blocks Purchase',async()=>{
  const{mod,state}=await setup({consent:true});await mod.initTracking();assert.ok(state.events.some(e=>e[0]==='track'&&e[1]==='PageView'));assert.ok(state.events.some(e=>e[0]==='track'&&e[1]==='ViewContent'));assert.equal(state.events.find(e=>e[1]==='ViewContent')[2].value,9.99);await mod.initCheckout();await state.paypalOptions.createOrder();mod.setConsent(false);await state.paypalOptions.onApprove({orderID:ORDER},{});assert.equal(state.calls.find(c=>c.url==='/api/capture').body.consent,false);assert.equal(state.events.some(e=>e[1]==='Purchase'),false);
});
...
test('countdown reaches zero, hides promotion, displays USD19.99 and rechecks the server',async()=>{
  ...
  const cleanup=await mod.initOffer();assert.equal(offerNodes.price.textContent,'$9.99');assert.equal(offerNodes.countdown.textContent,'00:00:01');
  Date.now=()=>now+1100;state.price='19.99';state.offerTick();await Promise.resolve();await Promise.resolve();
  assert.equal(offerNodes.price.textContent,'$19.99');assert.equal(offerNodes.countdown.textContent,'00:00:00');assert.equal(offerNodes.promo.hidden,true);
...
test('price expiry during checkout requires review before any order is created',async()=>{
  const{mod,state,map}=await setup();await mod.initCheckout();state.price='19.99';
  ...
  assert.equal(state.calls.find(call=>call.url==='/api/orders').body.expectedPrice,'19.99');
});
...
test('an existing discounted order stays USD9.99 while public course labels show USD19.99',async()=>{
  const{mod,state,offerNodes}=await setup({price:'19.99',acceptedPrice:'9.99'});await mod.initCheckout();await state.paypalOptions.createOrder();
  assert.equal(offerNodes.price.textContent,'$19.99');assert.equal(state.calls.find(call=>call.url==='/api/orders').body.expectedPrice,'9.99');
});

test('initial price lookup failure clears stale promotional amounts and retries when the page regains focus',async()=>{
  const{mod,state,offerNodes}=await setup({price:'19.99'});offerNodes.price.textContent='$9.99';state.configFailure=true;
  ...
  assert.equal(offerNodes.price.textContent,'$19.99');assert.equal(offerNodes.promo.hidden,true);
...
test('historical checkout amounts cannot be advertised as current checkout prices',async()=>{
  for(const price of ['19.00','29.00','29.99','49.99','99.00']){
...
test('a server-accepted historical USD99 order keeps its amount through capture and Meta Purchase',async()=>{
  const{mod,state,map,offerNodes}=await setup({consent:true,price:'9.99',acceptedPrice:'99.00'});
  await mod.initCheckout();await state.paypalOptions.createOrder();await state.paypalOptions.onApprove({orderID:ORDER},{});
  assert.equal(offerNodes.price.textContent,'$9.99');
...
test('verified historical USD99 access displays the original receipt without a new order or replayed Purchase',async()=>{
  const{mod,state,map}=await setup({existing:true,consent:true,price:'19.99',acceptedPrice:'99.00'});
...
test('server-accepted historical USD19, USD29, USD29.99 and USD49.99 orders preserve their amounts after the price migration',async()=>{
  for(const acceptedPrice of ['19.00','29.00','29.99','49.99']){
    const{mod,state,map,offerNodes}=await setup({consent:true,price:'19.99',acceptedPrice});
    await mod.initCheckout();assert.equal(offerNodes.price.textContent,'$19.99');
    await state.paypalOptions.createOrder();
...
test('verified historical USD19, USD29, USD29.99 and USD49.99 access is restored without another order or replayed Purchase',async()=>{
  for(const acceptedPrice of ['19.00','29.00','29.99','49.99']){
