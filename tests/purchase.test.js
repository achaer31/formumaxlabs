...
async function setup({existing=false,pending=false,consent=false,badConfig=false,badUrl=false,failedSdk=false,price='29.00',acceptedPrice=null}={}){
  ...
  const fetchMock=async(url,options)=>{
    ...
    if(url==='/api/config')return Response.json({productId:'ultimate-video-ai-mastery',price:badConfig?'1.00':state.price,regularPrice:'99.00',offerPrice:'29.00',offerActive:state.price==='29.00',offerExpiresAt:state.offerExpiresAt||new Date(Date.now()+(state.price==='29.00'?18000000:-1000)).toISOString(),serverTime:new Date(Date.now()).toISOString(),acceptedOrder:state.acceptedPrice?{orderId:ORDER,price:state.acceptedPrice,currency:'USD'}:null,currency:'USD',checkoutAvailable:true,paypalClientId:'mock-client',pixelId:'1467482891263700'});
...
test('countdown reaches zero, hides promotion, displays USD99 and rechecks the server',async()=>{
  ...
  const cleanup=await mod.initOffer();assert.equal(offerNodes.price.textContent,'$29');assert.equal(offerNodes.countdown.textContent,'00:00:01');
  Date.now=()=>now+1100;state.price='99.00';state.offerTick();await Promise.resolve();await Promise.resolve();
  assert.equal(offerNodes.price.textContent,'$99');assert.equal(offerNodes.countdown.textContent,'00:00:00');assert.equal(offerNodes.promo.hidden,true);
...
test('price expiry during checkout requires review before any order is created',async()=>{
  const{mod,state,map}=await setup();await mod.initCheckout();state.price='99.00';
...
test('USD99 purchases report the captured value and display the actual receipt total',async()=>{
  const{mod,state,map}=await setup({consent:true,price:'99.00'});await mod.initCheckout();await state.paypalOptions.createOrder();await state.paypalOptions.onApprove({orderID:ORDER},{});
  assert.equal(state.events.find(event=>event[1]==='Purchase')[2].value,99);
  assert.ok(map.get('#checkout-content').children.some(node=>node.textContent?.includes('Paid US$99')));
});
test('an existing discounted order stays USD29 while public course labels show USD99',async()=>{
  const{mod,state,offerNodes}=await setup({price:'99.00',acceptedPrice:'29.00'});await mod.initCheckout();await state.paypalOptions.createOrder();
  assert.equal(offerNodes.price.textContent,'$99');assert.equal(state.calls.find(call=>call.url==='/api/orders').body.expectedPrice,'29.00');
});

test('initial price lookup failure clears stale promotional amounts and retries when the page regains focus',async()=>{
  const{mod,state,offerNodes}=await setup({price:'99.00'});offerNodes.price.textContent='$29';state.configFailure=true;
  ...
  assert.equal(offerNodes.price.textContent,'$99');assert.equal(offerNodes.promo.hidden,true);
